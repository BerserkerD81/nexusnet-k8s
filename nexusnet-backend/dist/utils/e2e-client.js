...
class E2EClient {
    ...
    async initConversation(conversationId) {
        const keyPair = await crypto.subtle.generateKey({ name: 'ECDH', namedCurve: 'X25519' }, true, ['deriveKey', 'deriveBits']);
     ...
     */
    async addPeerKey(conversationId, peerUserId, peerPublicKeyB64) {
        const bundle = this.getBundle(conversationId);
        const peerKeyBuffer = base64ToArrayBuffer(peerPublicKeyB64);
        const peerPublicKey = await crypto.subtle.importKey('raw', peerKeyBuffer, { name: 'ECDH', namedCurve: 'X25519' }, false, []);
        // Derive shared bits via ECDH
        const sharedBits = await crypto.subtle.deriveBits({ name: 'ECDH', public: peerPublicKey }, bundle.ownKeyPair.privateKey, 256);
     ...
     */
    async encrypt(conversationId, recipientUserId, plaintext) {
     ...
     */
    async decrypt(conversationId, senderUserId, payload) {
    ...
    // ─── Helpers ───────────────────────────────────────────────────────────────
    getBundle(conversationId) {
        const bundle = this.conversations.get(conversationId);
        if (!bundle)
            throw new Error(`No key bundle for conversation ${conversationId}. Call initConversation() first.`);
        return bundle;
    }
    getSharedKey(conversationId, userId) {
        const bundle = this.getBundle(conversationId);
        const key = bundle.sharedKeys.get(userId);
        if (!key)
            throw new Error(`No shared key with user ${userId} in conversation ${conversationId}. Call addPeerKey() first.`);
        return key;
    }
}
exports.E2EClient = E2EClient;
// ── Utilities ────────────────────────────────────────────────────────────────
