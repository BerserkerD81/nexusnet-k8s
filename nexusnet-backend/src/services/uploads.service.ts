...
export class UploadsService {
   ...
   */
  async processImageFile(file: Express.Multer.File): Promise<string> {
   ...
   */
  async processImageFiles(files: Express.Multer.File[]): Promise<string[]> {
   ...
   */
  validateImages(files: Express.Multer.File[] | undefined): void {
    if (!files || files.length === 0) return;
