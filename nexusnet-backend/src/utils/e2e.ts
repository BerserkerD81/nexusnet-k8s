 ...
 */
export function deriveSymmetricKey(sharedSecret: Buffer, conversationId: string): Buffer {
  ...
  const prk = createHash('sha256').update(Buffer.concat([salt, sharedSecret])).digest();
